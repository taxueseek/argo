...
def super_search(query: str, engine: str = "auto", n: int = 5, explain: bool = False,
    ...
    Args:
        ...
        plan_only: 仅离线计划，不联网
        force_search: 即使判定 known-url 也强制多引擎搜索（不推荐）
        envelope: 附加 candidates/coverage/limitations（默认 True，向后兼容多字段）
        context: search | research（research 由 deep_research 传入）
    """
